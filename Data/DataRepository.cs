...
namespace backend.Data
{
    public class DataRepository : IDataRepository
    {
        ...
        public DataRepository(IConfiguration configuration) => this._connectionString = configuration["ConnectionStrings:DefaultConnection"];

        public async Task<QuestionGetSingleResponse> GetQuestion(Int32 questionId)
        {
            using (var connection = new SqlConnection(this._connectionString))
            {
                    ...
                    new { QuestionId = questionId }))
                {
                    ...
                    if (question != null) question.Answers = (await results.ReadAsync<AnswerGetResponse>()).ToList();
                    return question;
                }
            }
        }
        ...
        public async Task<IEnumerable<QuestionGetManyResponse>> GetUnansweredQuestionsWithPaging(int pageNumber, int pageSize)
        {
            using (var connection = new SqlConnection(this._connectionString))
            {
                await connection.OpenAsync();
        ...
        public async Task<Boolean> QuestionExists(Int32 questionId)
        {
            using (var connection = new SqlConnection(this._connectionString))
            {
                await connection.OpenAsync();
        ...
        public async Task<QuestionGetSingleResponse> PutQuestion(Int32 questionId, QuestionPutRequest question)
        {
            using (var connection = new SqlConnection(this._connectionString))
            {
                await connection.OpenAsync();
        ...
        public async Task DeleteQuestion(Int32 questionId)
        {
            using (var connection = new SqlConnection(this._connectionString))
            {
                await connection.OpenAsync();
        ...
        public async Task<AnswerGetResponse> GetAnswer(Int32 answerId)
        {
            using (var connection = new SqlConnection(this._connectionString))
            {
                await connection.OpenAsync();
